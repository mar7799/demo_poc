...
let capturedScreenshots = []; // Buffer for multi-screenshot capture
...
async function initializeGemini(profile = 'interview', language = 'en-US') {
    ...
    if (apiKey) {
        ...
        const success = await ipcRenderer.invoke('initialize-gemini', apiKey, buildContext(prefs), profile, language);
        if (success) {
...
async function initializeLocal(profile = 'interview') {
    ...
    const success = await ipcRenderer.invoke('initialize-local', ollamaHost, ollamaModel, whisperModel, profile, buildContext(prefs));
    if (success) {
        metaMaxPro.setStatus('Local AI Live');
        return true;
    } else {
        metaMaxPro.setStatus('error');
        return false;
...
async function initializeCloud(profile = 'interview') {
    ...
    const prefs = await storage.getPreferences();
    const success = await ipcRenderer.invoke('initialize-cloud', token, profile, buildContext(prefs));
    if (success) {
        metaMaxPro.setStatus('Live');
        return true;
    } else {
...
async function _captureFrameAsBase64(quality = 'medium') {
    ...
    const destH = srcW > MAX_WIDTH ? Math.round(srcH * (MAX_WIDTH / srcW)) : srcH;
    ...
    return new Promise(resolve => {
        offscreenCanvas.toBlob(blob => {
            ...
            reader.onloadend = () => {
                ...
                resolve(b64 && b64.length >= 100 ? b64 : null);
            };
            reader.readAsDataURL(blob);
...
async function captureManualScreenshot(imageQuality = null) {
    ...
    if (!mediaStream) { console.error('No media stream available'); return; }

    const b64 = await _captureFrameAsBase64(imageQuality || currentImageQuality);
    if (!b64) { console.error('Failed to capture frame'); return; }
    ...
    if (!result.success) {
        ...
        metaMaxPro.updateCurrentResponse(`Error: ${result.error}`);
    }
}
...
async function captureScreenshotToBuffer(imageQuality = null) {
    if (!mediaStream) { console.error('No media stream available'); return capturedScreenshots.length; }
    const b64 = await _captureFrameAsBase64(imageQuality || currentImageQuality);
    if (b64) {
        capturedScreenshots.push(b64);
        console.log(`Screenshot added to buffer (${capturedScreenshots.length} total)`);
    }
    return capturedScreenshots.length;
}

// Analyse all buffered screenshots together as a coding exercise
async function analyzeWithCapturedScreenshots(imageQuality = null) {
    if (capturedScreenshots.length === 0) {
        // Fallback: single live screenshot
        captureManualScreenshot(imageQuality);
        return;
    }
...
window.captureScreenshotToBuffer = captureScreenshotToBuffer;
window.analyzeWithCapturedScreenshots = analyzeWithCapturedScreenshots;
window.getCapturedScreenshotCount = () => capturedScreenshots.length;
window.clearCapturedScreenshots = () => { capturedScreenshots = []; };
...
const metaMaxProApp = document.querySelector('meta-max-pro-app');
...
// Consolidated metaMaxPro object - all functions in one place
const metaMaxPro = {
    // App version
    getVersion: async () => ipcRenderer.invoke('get-app-version'),

    // Element access
    element: () => metaMaxProApp,
    e: () => metaMaxProApp,

    // App state functions - access properties directly from the app element
    getCurrentView: () => metaMaxProApp.currentView,
    getLayoutMode: () => metaMaxProApp.layoutMode,
