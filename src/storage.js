...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'meta-max-pro-config');
    } else if (platform === 'darwin') {
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'meta-max-pro-config');
    } else {
        configDir = path.join(os.homedir(), '.config', 'meta-max-pro-config');
    }
