...
export class MainView extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            const [prefs, creds] = await Promise.all([
                ...
                metaMaxPro.storage.getCredentials().catch(() => ({})),
            ]);
    ...
    async _saveMode(mode) {
        ...
        this._tokenError = false;
        this._keyError = false;
        await metaMaxPro.storage.updatePreference('providerMode', mode);
        this.requestUpdate();
    }
    ...
    async _saveToken(val) {
        ...
        this._tokenError = false;
        try {
            const creds = await metaMaxPro.storage.getCredentials().catch(() => ({}));
            await metaMaxPro.storage.setCredentials({ ...creds, cloudToken: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveGeminiKey(val) {
        this._geminiKey = val;
        this._keyError = false;
        await metaMaxPro.storage.setApiKey(val);
        this.requestUpdate();
    }

    async _saveGroqKey(val) {
        this._groqKey = val;
        await metaMaxPro.storage.setGroqApiKey(val);
        this.requestUpdate();
    }
    ...
    async _saveOpenaiKey(val) {
        ...
        try {
            const creds = await metaMaxPro.storage.getCredentials().catch(() => ({}));
            await metaMaxPro.storage.setCredentials({ ...creds, openaiKey: val });
        } catch (e) {}
        this.requestUpdate();
    }
            ...
            <div class="form-wrapper">
                ...
                <div class="page-subtitle">
                    ${this._mode === 'cloud' ? 'Enter your invite code to get started' :
                      ...
                      'Use Claude as your interview AI'}
                </div>
