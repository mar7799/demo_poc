...
export class CustomizeView extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            const [prefs, keybinds] = await Promise.all([metaMaxPro.storage.getPreferences(), metaMaxPro.storage.getKeybinds()]);
    ...
    getDefaultKeybinds() {
        const isMac = metaMaxPro.isMacOS || navigator.platform.includes('Mac');
        return {
    ...
    getKeybindActions() {
        return [
            ...
            { key: 'startSession', name: 'Start Session', description: 'Start a new session from the main screen' },
            ...
            { key: 'pinLeft', name: 'Pin Navigate Left', description: 'Cycle left through pinned design/code chips at the bottom' },
            { key: 'pinRight', name: 'Pin Navigate Right', description: 'Cycle right through pinned design/code chips at the bottom' },
    ...
    async handleAudioModeSelect(e) {
        ...
        await metaMaxPro.storage.updatePreference('audioMode', this.audioMode);
        this.requestUpdate();
    }
    ...
    async handleGoogleSearchChange(e) {
        ...
        await metaMaxPro.storage.updatePreference('googleSearchEnabled', this.googleSearchEnabled);
        if (window.require) {
            try {
                const { ipcRenderer } = window.require('electron');
    ...
    async handleBackgroundTransparencyChange(e) {
        ...
        await metaMaxPro.storage.updatePreference('backgroundTransparency', this.backgroundTransparency);
        this.updateBackgroundAppearance();
        this.requestUpdate();
    }
    ...
    updateBackgroundAppearance() {
        ...
        metaMaxPro.theme.applyBackgrounds(colors.background, this.backgroundTransparency);
    }

    async handleFontSizeChange(e) {
        this.fontSize = parseInt(e.target.value, 10);
        await metaMaxPro.storage.updatePreference('fontSize', this.fontSize);
        this.updateFontSize();
        this.requestUpdate();
    }
    ...
    async restoreAllSettings() {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(defaults)) {
                await metaMaxPro.storage.updatePreference(key, value);
            }
    ...
    renderAudioSection() {
        return html`
            <section class="surface">
                <div class="surface-title">Audio & Screen Capture</div>
                <div class="form-grid">
